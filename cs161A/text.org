...
The =-std=c++17= part tells =g++= which version of C++ to use, and =-Wall= turns on many helpful warnings.
...
You've made it through the earlier CS161A material. In this slower-paced sequence, we intentionally keep some overlap with CS161B by introducing a first pass at a few thornier topics here.

Why include these later topics in CS161A? A few reasons:

1. *To slow down the first encounter*: Some topics benefit from being introduced gently here and then revisited with more depth later.

2. *To show how the early material grows*: Arrays, strings, functions, and references all lead naturally into the topics below.

3. *To provide overlap on thorny concepts*: Vectors, file I/O, and program organization are easier to absorb when they appear more than once across the sequence.

4. *To support bridge students*: If you're taking CS161A to prepare for CS161B, this overlap gives you a useful runway.
...
Important: Don't feel pressured to master these topics on the first pass. They are part of CS161A in this sequence, but CS161B will revisit them with more depth, more practice, and more demanding assignments.

That said, if you're feeling confident with the earlier CS161A material, by all means dive in. These are all practical, useful topics that will make you a better programmer.

Think of this as an intentional bridge: enough exposure to make the second pass more meaningful later.
...
[fn:9] This is only true for memory allocated, allocated being the technical term for setting up the dividers on the storage wall and putting the labels on the cubbyhole, /on the stack/. All the memory you're allocating in this class---every variable you create, every array you make---will be on the stack. There is /another/ way to allocate memory that is more flexible but also harder to manage, usually called "the heap", but it still only lasts as long as the program is running. If you forget to clean it up, that's called a "memory leak": the memory stays in use until the program ends, which is one reason programs like Chrome and Firefox can seem to swell up to absurd amounts of RAM if you leave them open long enough.
