...
#+begin_src cpp
  cout << "Hello, World" << endl;
#+end_src
as you can probably guess is what actually prints the line "Hello, World". Think of =cout= as the /target/ you're sending text to to be printed out. You send things to it with the =<<= symbol. The text you're sending is in quotation marks. In C++, like most programming languages, you denote text-as-data with double-quotes. The other thing we need to explain here is the =endl=, which is just the way of saying "end this line and start a new one".
...
3. *Challenge*: Build a single-item sales receipt. Ask for the unit price, the quantity, and the sales-tax rate (as a percentage, e.g. =8.5= for 8.5%). Compute and print the subtotal (price × quantity), the tax (subtotal × rate ÷ 100), and the grand total — each on its own line. Don't worry if the decimal places look a little ragged; making dollar amounts line up nicely is exactly what the Data Formatting section later in this chapter is for, and you can come back and polish this program afterwards.
...
Quick Check: What happens if the longest word is longer than the space you set aside for it? Does it truncate or does it throw off the formatting? Try it and find out!
...
Try to read this code through and understand the logic of what it's doing! The only thing we haven't explained to this point is the =%= operator, which is the "modulus" operator. Basically, =a % b= returns the remainder---in the "you're learning division as a kid" sense of remainder---of dividing =a= by =b=. Now, in terms of why we *care* about this operator it gives us a quick way to take a big range of numbers and map it to something smaller. For example, if we do =a % 20= our outputs, for any non-negative =a=, will be between 0 and 19. Similarly if we say =a % 100= this will give us an output that's between 0 and 99. (For /negative/ numbers, C++'s =%= gives back negative remainders---=-5 % 20= is =-5=, not =15=---but since =rand()= never hands us a negative number we don't have to worry about that here.)
...
We'll cover, in greater depth, /why/ C++ doesn't protect you from this error but for now the way you should think of it is that when you declare an array variable you're actively setting aside a contiguous set of cubbyholes in the storage wall, but that *you* need to keep track of which ones belong to which array. This is part of the reason why, in our previous example, we had a separate constant called =numPets= that kept track of how many pets were supposed to be in the database. Remember from a moment ago that a /constant/ is just a variable with =const= in front of its declaration, meaning that once it has a value that value isn't allowed to change. That's exactly the property you want for a piece of bookkeeping the compiler can't do for you: the size of the array can't drift out of sync with the loops that use it if nothing is allowed to change it. Here's the pet database again, tangled out so you can experiment with it:
...
You can also use a =switch= statement for menus, which is often cleaner. We haven't officially met =switch= yet --- it gets its own proper treatment in the "Switches vs. ifs" section a couple of chapters from now --- but the shape is simple enough to preview here: =switch(choice)= means "look at the value of =choice=," each =case 1:= line means "if it was 1, start running here," the =break= means "okay, done with this option, jump past the whole thing," and =default:= is the catch-all like a final =else=. If it doesn't fully click yet, feel free to just use the if/else version above; they do the same job.
...
#+end_example
What you'll get printed out is "The sum of these numbers was: 1". Why? Because the attempt to read =dog= into =num2= fails, so =cin= goes into an error state and writes a 0 into =num2=. The sum winds up being 1 + 0.

That detail matters. When a read fails, =cin >>= zeroes out the variable and refuses to do anything further until you acknowledge the error --- it does /not/ quietly skip the bad input and move on. (If you're using a truly ancient compiler, from before the 2011 version of C++, the variable was instead left untouched --- one more reason we always initialize our variables.)
...
2. *Intermediate*: Ask for a test score between 0 and 100, re-prompting on non-numeric input /and/ on numbers outside the range. Once you have a valid score, print the letter grade (90+ is an A, 80-89 a B, and so on).
...
Okay, also we should talk about how to turn data into strings because this is going to be useful! Basically, there's just a function in the =<string>= library that you can use called =to_string= that will convert any other data to being a string. This works with basically every /numeric/ type, like in this program. (One fun trap to know about: if you hand =to_string= a =char=, you get the string form of its numeric character code --- =to_string('d')= is ="100"=, not ="d"= --- because =char= is secretly a small number and =to_string= takes it at its word.)
...
Also notice the =auto= for =atPosition=: remember that =find= hands back that special "type C++ uses for string positions" we talked about, and letting the compiler figure out the type is exactly the tool for "give me whatever type this function returns." Writing =int= there instead would only /appear/ to work, and your compiler would grumble at you about it with =-Wall= on.
...
Now remember how I showed you a bit about how arrays work and the fact that for-loops were made to work with them? Okay, so part of why I needed to show you that is that even in the stretches of this class where we aren't working with arrays directly, the /idea/ behind them keeps paying rent: you can treat strings "like" arrays for operating on them.
...
A small heads-up before you compile this yourself: if you're following the Preface's advice and compiling with =-Wall=, loops like this will earn you a warning about "comparison of integer expressions of different signedness." That's the compiler pointing out that =i= is an =int= while =s.length()= gives back that unsigned string-position type we keep mentioning. For strings of any sane length the loop is perfectly fine and everybody writes it this way while learning; the warning is a nudge, not an error, and the honest explanation of signed versus unsigned is waiting in the memory chapter.
...
** Switches vs. ifs
We got a sneak preview of =switch= back in the menu-driven programs section; now it's time to actually understand it. Switch statements are actually pretty simple and can, essentially, be thought of as "compiling" down to a more verbose if-elseif-else statement, except that rather than being able to ask *any* question you're just asking the question is /this/ equal to /this/.
...
+ passing an array to a function really means passing the address, which is why functions that take arrays have to agree on some other convention (a sentinel, or an explicit length parameter) to know when to stop --- we'll write a function called =dynLength= in the very next section that does exactly this
...
We won't use vectors much in the core of this class --- they get their proper introduction in the Extended Topics chapter --- and that's because we're trying to install +fear of+ healthy respect for the problems that can happen if you aren't careful using arrays.
...
Here is the honest version. When C++ makes an =int=, it sets aside a /fixed/ amount of space — on almost every machine you're likely to use, that's 32 bits, though other sizes exist. That means an =int= can only represent a fixed, finite range of whole numbers. It is not the integers from your math class; it is a finite window onto them. If you try to add two =int= s and the true result doesn't fit in that window, you've caused what's called "overflow," and it is the real reason I was cagey about whether =int= was really the whole numbers. What you'll /usually observe/ when a plain =int= overflows is the stored answer wrapping around, the way a car odometer rolls over from its biggest reading back to zero --- but I have to be honest here too: for signed types like =int=, the C++ standard doesn't promise the wraparound. It calls overflowing an =int= /undefined behavior/, which means the compiler is allowed to assume it never happens. Only the /unsigned/ types we're about to meet get the odometer roll guaranteed in writing.
...
One detail that's easy to miss but /very/ important: look at the declaration =char hello[20]=. We had to make the destination array bigger than the text we initially put in it, because =concat= writes the second string into the same box the first string lives in. If we'd declared =char hello[] = "Hello"= the compiler would have made a box exactly six characters wide (five letters plus the ='\0'=), and copying ", World!" onto the end would have written right past the end of the array --- exactly the out-of-bounds sin we just spent a whole section warning you about. The destination needs spare capacity for everything you plan to append. C++ will not check this for you.
...
Oh, and one bookkeeping note: see the =};= at the end of the struct definition? Way back at the start of this book I told you that lines ending in ={= or =}= don't need semicolons, "with one annoying exception we'll get to." This is it. A struct definition is secretly a /statement/ (it declares a type), so its closing brace needs the semicolon that every statement gets. Forget it and the compiler will produce some truly baffling error messages pointing at whatever innocent line comes next.
...
Notice the guard at the top: if both inputs are zero, =gcd= hands back zero, and dividing by zero doesn't produce an error message you can read --- it kills the program outright. When a function divides by something another function computed, it's worth asking "can that ever be zero?" Here, it can, so we handle it.
...
Remember, back in the very first chapter --- when we were still echoing text back at ourselves --- when I joked that "we're very good programmers and *never* forget to put =return 0= in our files"? I promised back then that we'd earn that joke eventually. This is the chapter where the joke stops being a joke.
...
Try this with bigger and bigger inputs and watch what happens somewhere around =n = 13=: factorials grow so fast that they escape the finite window an =int= can represent, and you'll see overflow with your own eyes long before the memory chapter explains it.
