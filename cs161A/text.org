...
Let's try writing some functions that will let you actually do something with this

#+begin_src cpp :tangle codesnips/dyanmicArray1.cpp
  #include <iostream>
  using namespace std;
...
Well at first all that's going to change is that we're going to make our code use an array of =char=.
...
The only real difference is that since we're using /arrays/ we have to explicitly pass the length around rather than just being able to /ask/ the array how long it is.

What if I told you, though, character arrays in C were a kind of /dynamic array/ like we made above?

Wait though, let's actually be a little clever and instead of 
