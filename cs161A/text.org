...
*** Proximity Detection: "Getting Warm!"
A nice feature in guessing games is to tell the player when they're getting close to the answer. We can do this by checking how far away the guess is from the target number.

To check the distance between two numbers, we need the /absolute value/ of their difference. The absolute value function =abs()= is available in C++ by including =<cmath>=.

Here's an enhanced guessing game with proximity hints:

#+begin_src cpp :tangle codesnips/warmCold.cpp
  #include <iostream>
  #include <cstdlib>
  #include <ctime>
  #include <cmath>
  ...
  int main(){
    srand(time(0));
    int target = rand() % 50 + 1;  // Random number 1-50
    int guess = 0;
...
The key here is =abs(target - guess)= which gives us the distance between the two numbers. If the guess is 42 and the target is 45, the distance is 3. If the guess is 48 and the target is 45, the distance is also 3.

We check the distance first (closest matches first) before checking if the guess is too high or too low.
...
Now for the real challenge: a centered pyramid. This requires /two/ inner loops: one for spaces and one for stars.

The pattern for a pyramid of height 5 looks like:
#+begin_example
    *
   ***
  *****
...
- Row 4 has 1 space and 7 stars
- Row 5 has 0 spaces and 9 stars

The pattern is:
- Number of spaces = height - row
- Number of stars = 2 * row - 1

Here's the code:

#+begin_src cpp :tangle codesnips/pyramid.cpp
  #include <iostream>
  using namespace std;
...
This is a classic nested loop pattern that you'll see in many programming courses!
...
Here's an example: counting dice roll results.
...
The key insight: we use the value we're counting as the /index/ into our counts array!

Here's a more complex example: counting the sum of two dice:

#+begin_src cpp :tangle codesnips/twoDiceFreq.cpp
  #include <iostream>
  #include <cstdlib>
  #include <ctime>
  using namespace std;
  ...
  int main(){
    ...
    int counts[13] = {0};  // Use indices 2-12

    // Roll two dice NUM_ROLLS times
    for(int i = 0; i < NUM_ROLLS; i++){
      int die1 = rand() % 6 + 1;
      int die2 = rand() % 6 + 1;
      int sum = die1 + die2;
    ...
    // Find the most and least frequent sums
    int maxCount = 0;
    int minCount = NUM_ROLLS;
    int maxSum = 0;
    int minSum = 0;
    ...
    cout << "---   -----" << endl;
    for(int sum = 2; sum <= 12; sum++){
      cout << sum << "     " << counts[sum] << endl;
    }

    cout << "\nMost frequent: " << maxSum << " (" << maxCount << " times)" << endl;
    cout << "Least frequent: " << minSum << " (" << minCount << " times)" << endl;

    double sevenPercent = (counts[7] * 100.0) / NUM_ROLLS;
    cout << "Percentage of 7s: " << sevenPercent << "%" << endl;

    return 0;
  }
#+end_src

This pattern is extremely useful for statistics, histograms, and data analysis!
...
Here's how to implement rotation:
...
#+begin_src cpp :tangle codesnips/arrayRotate.cpp
  ...
  void rotateRight(int arr[], int size, int positions){
    // Normalize positions (rotating by size is same as no rotation)
  ...
  int main(){
    ...
    rotateRight(numbers, SIZE, rotateBy);
...
The key to rotation is the formula: =newIndex = (i + positions) % size=. The modulo operator makes the wraparound work!
...
#+end_src
*** Caesar Cipher: Shifting Letters with Wraparound
...
The tricky part is handling the wraparound (what happens when you go past 'z') and preserving the case of letters.

Here's how to implement a Caesar cipher encoder:
...
#+begin_src cpp :tangle codesnips/caesarCipher.cpp
  ...
  char shiftChar(char ch, int shift){
    ...
    // Shift the character
    // We subtract base to get 0-25, add shift, use % 26 to wrap around
    // then add base back to get the ASCII value
  ...
  int main(){
    ...
    string decoded = caesarEncode(encoded, 26 - shift);
...
The key insight is in =shiftChar=:
1. We check if the character is a letter (if not, return it unchanged)
2. We find the "base" (either 'A' or 'a') based on the case
3. We do math: =(ch - base + shift) % 26=
   - =ch - base= converts the letter to a number 0-25
   - =+ shift= shifts it forward
   - =% 26= makes it wrap around (so 27 becomes 1)
   - =+ base= converts back to an ASCII character

The modulo operator (=%= ) is what makes the wraparound work!

If you run this with "Hello World!" and a shift of 3, you get "Khoor Zruog!"

*** Exercises on Ciphers
1. *Basic*: Modify the Caesar cipher to also handle decoding (shifting backwards) as a separate menu option.
2. *Intermediate*: Create a ROT13 cipher (shift of 13) which is its own inverse (encoding twice gives you the original text).
3. *Challenge*: Implement a frequency analysis tool that counts how often each letter appears in encrypted text (useful for breaking Caesar ciphers!).
...
A classic use of bool functions is checking if a number is prime:

#+begin_src cpp :tangle codesnips/isPrime.cpp
  #include <iostream>
  #include <cmath>
  using namespace std;
...
Notice how we can use =isPrime(i)= directly in an if-statement! This makes the code very readable: "if i is prime, do something."
