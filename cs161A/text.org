...
* TODO Arrays: Storing Multiple Values
** TODO Introduction to Arrays
*** Why Arrays are Needed
When you need to work with multiple related values (like storing grades for an entire class, or temperatures for each day of the week), creating individual variables becomes impractical. Arrays let us store multiple values of the same type under a single name.
...
int values[] = {1, 2, 3, 4, 5, 6}; // size is 6
#+end_src

** TODO Accessing Array Elements
Array indices start at 0 in C++. The first element is =arr[0]=, the second is =arr[1]=, and so on.

#+begin_src cpp
int temps[7] = {65, 68, 72, 75, 73, 70, 67};

cout << "Monday's temperature: " << temps[0] << endl;
cout << "Friday's temperature: " << temps[4] << endl;

// Modifying an element
temps[0] = 66; // Change Monday's temp
#+end_src

*** Exercises
1. *Basic*: Create an array of 5 test scores, initialize them, and print each one.
2. *Intermediate*: Ask the user to enter 5 numbers, store them in an array, then print them in reverse order.

** TODO Iterating Through Arrays with Loops
The combination of arrays and loops is fundamental to programming.
...
*** Exercises
1. *Basic*: Write a program that finds the maximum value in an array.
2. *Intermediate*: Count how many values in an array are above the average.
3. *Challenge*: Find the second-largest value in an array.

** TODO Arrays and Functions
*** Passing Arrays to Functions
Arrays are passed by reference to functions (unlike simple variables). This means changes to the array inside the function affect the original array.
...
*** Why Pass the Size?
Arrays don't "know" their own size when passed to functions, so you must pass the size as a separate parameter.

*** Exercises
1. *Basic*: Write a function that calculates the average of an array.
2. *Intermediate*: Write a function that reverses an array in-place.

** TODO Common Array Algorithms
...
*** Exercises
1. *Basic*: Implement a function to find the minimum value.
2. *Intermediate*: Implement a function that counts how many times a value appears in an array.
3. *Challenge*: Implement a function that removes duplicates from an array.

** TODO Two-Dimensional Arrays
For storing data in a grid or table format (like a tic-tac-toe board or grade spreadsheet).
...
cout << matrix[2][3] << endl; // Prints 12

// Nested loops to process
...
*** Exercises
1. *Basic*: Create a 3x3 array and fill it with values 1-9.
2. *Intermediate*: Write a function to sum all elements in a 2D array.
3. *Challenge*: Implement a simple tic-tac-toe board checker.

** TODO Array Limitations and Pitfalls
*** Arrays Cannot Change Size
Once declared, an array's size is fixed. You cannot add or remove elements.

*** Array Bounds and Buffer Overflow
C++ does NOT check array bounds. Accessing =arr[10]= when the array has only 5 elements will cause undefined behavior (crashes, corrupted data, security vulnerabilities).

#+begin_src cpp
int arr[5] = {1, 2, 3, 4, 5};
arr[10] = 99; // DANGER! Out of bounds - undefined behavior
#+end_src

Always ensure your indices stay within valid range: =0= to =size-1=.

*** Exercises
1. *Discussion*: What happens if you access an array out of bounds? Try it and observe.
2. *Basic*: Write code that safely checks bounds before accessing an array element.
...
*** LAB: Array Reversal
Write a function that reverses an array in place (without creating a new array).

*** LAB: Matrix Operations
Create a program with a 3x3 matrix that can:
- Display the matrix
...
- Dynamic sizing (can grow and shrink)
- Bounds checking (with =.at()= method)
- Built-in size tracking
- Easier to use with functions
...
When you have related pieces of information (like a student's name, ID, and GPA), keeping them in separate variables becomes unwieldy. Structs let you bundle related data together.
...
*** Why Use Files?
- Permanent storage (data persists after program ends)
