...
The =find= function returns a value in an /unsigned/ index type (meaning it can only hold positive numbers, or zero). However, =-1= is obviously a negative number. So what happens when we write =-1= but C++ tries to interpret it as that unsigned position type?
...
The =-1= gets converted to that unsigned index type, which turns it into that giant number =18446744073709551615= (on most modern systems), which is the same value as =string::npos=!

So this shorthand /works/, but it's still worse style than writing what you mean directly. In this text, prefer

#+begin_src cpp
  if(str.find("test") == string::npos)
#+end_src

because it is clearer and doesn't make the reader remember a signed/unsigned conversion trick.
...
- *Bounds checking*: The =.at()= method checks the index and notices bad accesses
...
When you're in bounds, these look almost identical. The difference only shows up when something goes wrong.

The =[]= operator behaves like old-school array indexing: fast, concise, and trusting. If you make a mistake, C++ does not protect you.

The =.at()= function asks the vector to check the index first. If the index is invalid, the program fails clearly. Early on, that tradeoff is often worth it.
...
These two loop styles are useful for different jobs.

Use an index-based loop when the /position/ matters: maybe you need to print the index, compare =numbers[i]= to =numbers[i + 1]=, or update one specific slot.

Use a range-based loop when you simply want to visit every element. In beginner code, that is often the clearer choice because it lets you talk directly about the /values/.
...
Notice the two different uses of references in this section.

In =printVector(const vector<int>& v)=, the =&= means "use the original vector," and the =const= means "I promise not to change it."

In =doubleValues(vector<int>& v)=, the reference is still there, but now the missing =const= is the whole point: the function is allowed to modify the caller's vector directly.

This is one of the places where vectors feel much nicer than built-in arrays. A function can simply /return/ a vector by value, and the caller gets a real container back with its size and contents intact. We don't have to separately pass a length, and we don't have to play pointer games just to move a list of numbers around.
...
This "build a result vector and return it" pattern shows up constantly. Read some input. Keep the pieces you want. Return the new collection. Once students get comfortable with that rhythm, a lot of data-processing programs start to feel much less mysterious.
...
is a compact way to fill in all the fields at once. The values go in the same order the fields were declared in the struct. That's convenient, but it does mean the order matters.
...
This section is also a good place to notice when we /copy/ a struct and when we /borrow/ one.

=calculateArea(Rectangle rect)= receives a copy. For a tiny struct like this, that's perfectly fine.

=displayRectangle(const Rectangle& rect)= receives a const reference. That avoids making another copy and promises the function will not change the caller's rectangle.
...
The nice part is that file I/O is much less exotic than it sounds. If =cin= and =cout= already make sense to you, then the basic flow here will feel familiar too.
...
If the rhythm of file I/O is starting to feel repetitive, excellent. This is one of those places where boring habits are exactly what you want:

1. connect a stream to a file
2. check that the connection worked
3. choose a reading pattern (=getline= for lines, =inFile >> word= for whitespace-separated chunks, and so on)
4. process each piece of data
...
*** Writing to Files
**** A first file-writing program
Reading files is half the story. The other half is producing them.
...
This should look comfortingly similar to the input example. That is deliberate. =ofstream= sends output to a file the same way =cout= sends output to the terminal.
...
Up to this point we have been cheerfully stuffing almost everything into one file, and for beginner programs that is honestly fine. But eventually one giant =main.cpp= stops feeling simple and starts feeling like you keep your entire life in one junk drawer.
...
Multi-file programs are how you stop doing that to yourself. Split the code up by responsibility. Put the public declarations in one place, the implementations in another, and let the files work together cleanly.

In CS161B, this stops being optional very quickly, so it is worth learning the mental model now: headers tell other files what exists, implementation files tell them how it works.

*** Why Split Code into Multiple Files?

Splitting code into multiple files buys you a few things immediately:
...
Header files usually contain declarations: what exists and how other files are allowed to use it.
...
If you read that pattern literally, it makes decent sense:

+ if this header's guard name has not been defined yet, define it now
+ then include the declarations in the file
+ if the header gets included again later, skip the body because the guard name is already defined

The syntax looks strange because it belongs to the preprocessor rather than ordinary C++. The idea is simple: include these declarations once.
...
Here's the big picture. =main.cpp= includes =math_utils.h= so the compiler knows the /names and types/ of the functions it is allowed to call. The actual bodies of those functions live in =math_utils.cpp=. When the program is linked, the linker matches the calls in =main.cpp= to the definitions in =math_utils.cpp=.

That split is the entire point of multi-file programs:
+ the header says what exists
+ the implementation file says how it works
+ =main.cpp= uses those pieces to build the program

*** Compiling Multi-File Programs
**** Command-Line Compilation
#+begin_example
# Compile all files together
g++ -std=c++17 -Wall -Wextra -pedantic main.cpp math_utils.cpp -o program

# Or compile separately (faster for large projects)
g++ -std=c++17 -Wall -Wextra -pedantic -c math_utils.cpp   # Creates math_utils.o
g++ -std=c++17 -Wall -Wextra -pedantic -c main.cpp         # Creates main.o
...
math_utils.o: math_utils.cpp math_utils.h
	g++ -std=c++17 -Wall -Wextra -pedantic -c math_utils.cpp
...
*** Common Errors with Multi-File Programs
...
Those two errors are common because they are almost opposites of each other.

"Multiple definition" means the program saw the same implementation more than once.

"Undefined reference" means the program saw a declaration or function call, but no compiled implementation ever showed up to satisfy it.

If you remember the division of labor, these messages become much less mysterious:

+ header files advertise what exists
+ implementation files provide the actual code
+ the build command has to include every implementation file the program needs
...
That list is pretty close to the actual shape of how most successful beginner programs get written.

One of the fastest ways to get stuck is to leap from "I sort of understand the assignment" straight into coding. Naming the inputs, outputs, edge cases, and subproblems first usually makes the implementation much calmer.
...
The real theme here is simple: debug on purpose.

When something breaks, change one thing, gather one new fact, and let that fact guide the next step. Random thrashing feels busy, but it rarely teaches you what actually went wrong.
