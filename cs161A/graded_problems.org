...
(Note: everything you need for the menu is the if/else-if chain from the
textbook's Foundations chapter --- you do not need =switch=, which the labs
cover later.)
...
- In main, provide a menu to choose which distance to calculate, numbered exactly:
  1. 2D Euclidean distance
  2. 3D Euclidean distance
  3. Manhattan distance
  (the autograder selects options by these numbers)
- Prompt for the appropriate coordinates, first point then second point (x before y before z)
...
- Implement the following functions:
  - =void hoursToMinutesSeconds(int hours, int& minutes, int& seconds)= - converts hours to total minutes and total seconds (2 hours → 120 minutes, 7200 seconds)
  - =void minutesToHoursSeconds(int minutes, int& hours, int& seconds)= - converts minutes to whole hours plus the left-over part expressed in seconds (90 minutes → 1 hour and 1800 seconds)
  - =void secondsToHoursMinutes(int seconds, int& hours, int& minutes)= - converts seconds to whole hours plus whole left-over minutes, discarding leftover seconds (3700 seconds → 1 hour and 1 minute)
- In main:
